...
func findFirstAndLastDigits(line string) (int, int) {
	...
	for linePosition, r := range line {
		...
		if !unicode.IsDigit(r) {
			if isDigit, value := isDigitString(line, linePosition); isDigit {
				if firstDigit == -1000 {
					firstDigit = value
				} else {
					lastDigit = value
...
func isDigitString(line string, index int) (bool, int) {
	for i, s := range textNumbers {
		if index+len(s) < len(line)+1 && line[index:index+len(s)] == s {
			return true, i + 1
		}
	}
